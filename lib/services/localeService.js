...
class LocaleService {
	static _obtainConfig(userConfig) {
		const config = _.extend(_.clone(defaultConfig), userConfig);
	...
	static _tryString(config, init, resourceIdentifier, locale) {
		if (!config.inited && init) init();
		if (!locale) return localeUtils.tryFromDefaultLocale(config.defaultLocalePath, resourceIdentifier);
		const localePath = localeUtils.obtainResources(locale, config);

		return localeUtils.obtainFromRequiredLocale(localePath, resourceIdentifier)
			|| localeUtils.tryFromDefaultLocale(config.defaultLocalePath, resourceIdentifier);
	}
}
